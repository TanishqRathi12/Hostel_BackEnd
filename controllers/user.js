...
export const joinHostelHandler = async (req, res) => {
  ...
  try {
    ...
    const checkStudent = await Student.findById(stId).select('Hostel').populate('Hostel', 'name');
    ...
    if (!checkStudent.Hostel) {
      ...
      const updatedStudent = await Student.findById(stId).populate('Hostel', 'name');
...
export const markAttendanceHandler = async (req, res) => {
  ...
  try {
    ...
    if (!student.Hostel) {
      return res.status(400).json({ error: 'Student is not assigned to any hostel' });
    }
